...
| 3 | Create `src/components/notification/styles/TimeSlotReminderNotification.tsx`: plain-div badge (`size-[72px] rounded-full bg-primary-98 border-2 border-primary-50` wrapping `<CalendarPlus className="text-primary-60" />`), message, CTA `<Button variant="fill" size="sm" fullWidth className="border border-primary-50">`, timestamp via `toLocaleDateString`, `leaf.svg` unseen slot | Card matches the Figma spec transcribed in issue #757 — **provisional, see risks** (no design image; the intended ground truth would be `.ai/references/757/design-01.png`) |
...
- **No design image.** `.ai/references/` does not exist and the screenshot pasted into chat could not be read, so sub-task 3's done-condition is unverifiable and `verify-ui` cannot run. The Figma spec dump in issue #757 is the provisional ground truth. Must be flagged on the PR.
